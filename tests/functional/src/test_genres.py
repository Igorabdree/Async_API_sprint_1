...
class TestGenres:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {'status': HTTPStatus.OK, 'length': 20, 'length_genre': 2}
            ),
        ]
    )
    @pytest.mark.asyncio
    async def test_get_genres(
        ...
        response = await make_get_genres_request(path='/')
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                {'status': HTTPStatus.NOT_FOUND, 'length': 5}
            ),
        ]
    )
    @pytest.mark.asyncio
    async def test_get_genres_on_id(
        ...
        response = await make_get_genres_request(path=f'/{path}')
